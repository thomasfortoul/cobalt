...
def getAssetPricesTotal(asset_id):
...
def get_median_hello(asset_id, median, max_median):
    # Find averages and initialize arrays.
    input_file = getAssetPricesTotal(asset_id);
...
def getAssetPrices(asset_id, limit):
    url = 'https://weave.rs/getAssetPrices.php'
    params = {
        'assetid': asset_id,
        'limit': limit
    ...
    try:
        ...
        if response.status_code == requests.codes.ok:
            ...
            for line in lines:
                ...
                if(len(values) != 6): # Check number of values of input.
                    logging.error(f'getAssetPrices error, {len(values)} values per line, expected 6.')
                    return empty_df
            ...
            #Check range of hello values.
                ...
                for hello_value in hello_values:
                    if((hello_value < 0.0) or (hello_value > 20.0)):
                        logging.error(f'getAssetPrices error, Hello values out of range: {hello_value}. Expected between 0, 20. Duplicating last entry.')
                        total_array = np.r_[total_array, [new_data]]
                        counter += 1
                        continue
                ...
                if (counter != 0):  # continue until data is caught up, keep taking last value.
                    if(next_date != date):
                        while (next_date != date):
                            ...
                            # append previous data (most recent)
                            new_data = np.array(
                                [prev_price, next_date.strftime("%d/%m/%y %H:%M:%S"), short, long, none])
                            total_array = np.r_[total_array, [new_data]]
                            next_date = next_date + difference
                    ...
                    new_data = np.array([prev_price, next_date.strftime("%d/%m/%y %H:%M:%S"), short, long, none])
                    total_array = np.r_[total_array, [new_data]]
            ...
            last_date = next_date - difference
            total_array[-1][1] = last_date.strftime("%d/%m/%y %H:%M:%S")
            logging.info(f'getAssetPrices finished. Last date = {last_date.strftime("%d/%m/%y %H:%M:%S")}')
            ...
            output = pd.DataFrame(data=total_array, columns=['Price', 'Date', 'Short', 'Long', 'None']).tail(
                -1).reset_index(drop=True)  # remove first empty row
...
def get_moyenne_mobiles(input_file, buffer_size, max_buffer, indices):
    ...
    current_row = max_buffer * buffer_size - 1  # 30*48-1 -> index of start point
    s_array = np.empty(len(indices), dtype=float)
    counter = 0
    for i in range(len(indices)):  # 0-4
        index = indices[i]# 1, 8, 15, 22
        tmp_sum = 0

        if(i == len(indices)): # for none, last index of indices.
            for k in range(buffer_size * (index + 1)):  # 0 - (48*max_buffer-1)
                tmp_sum += none.iloc[current_row - k]

        else:
            for k in range(buffer_size * (index + 1)):  # 0 - (48*max_buffer-1)
                tmp_sum += hello.iloc[current_row - k]  # going backwards in records, from current to 0 for last iteration

        s_array[i] = tmp_sum # [sum-1*48 (index1), sum-2*48 (index2), ..., (none index)]

    logging.info('Moyennes mobiles finished calculation.')
    return pd.DataFrame(data=s_array)
...
def main_func(argv):
    ...
    for model in models:
        ...
        limit = max_buffer * buffer_size + 1 + median

        logging.info(f'Limit = {limit}. Calling getAssetPrices.')

        # Feature engineering and extract batch ID
        output = getAssetPrices(asset_id, limit)
        if (output.empty):
            logging.error(f'getAssetPrices failed, skipping this model.')
            continue
        ...
        median = 48
        none_week = 2
        print(f'Details: {start}, {step}, {moyennes_mobiles}')
        output.to_csv('output.csv')
        output['Short'] = pd.to_numeric(output['Short'])
        output['None'] = pd.to_numeric(output['None'])
        output['Long'] = pd.to_numeric(output['Long'])
        tmp_file = get_median_hello(output, median, 48)
        tmp_file = tmp_file.tail(-48).reset_index(drop=True)  # Truncate for maximum median.
        print(tmp_file)
        ...
        try:
            moyennes_storage = getFeatureRow(output, buffer_size, max_buffer, 'Hello')
            indices = getIndices(step, start, moyennes_mobiles)
            sentiments = getFeatures(moyennes_storage, indices)
        ...
        try:
            ...
            logging.error(f'Unsuccessful getPrediction. Model: {model_id}. Skipping this model.')
            continue
